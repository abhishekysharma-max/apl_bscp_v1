...
import { DASHBOARD_CARDS } from '../constants/auth.config';
...
export class CommonService {
  ...
  ssoLogin() {
    const url =
      `https://login.microsoftonline.com/${AUTH_CONFIG.tenantId}/oauth2/v2.0/authorize` +
  ...
  getDashboardCards(role: string) {
    return DASHBOARD_CARDS[role as keyof typeof DASHBOARD_CARDS] || [];
  }
}
